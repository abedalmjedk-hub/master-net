...
# ضبط وقت أقصى للاتصال لمنع التعليق
...
def connect_imap(exit_on_fail=True):
    ...
    while True:
        try:
            ...
            try:
                ...
                mail = connect_imap(exit_on_fail=False)
                if mail:
                    mail.select("inbox")
                else:
            ...
            # تنظيف قائمة الإيميلات المعالجة لتجنب امتلاء الذاكرة
            if len(manager.processed_emails) > 2000:
                manager.processed_emails.clear()
